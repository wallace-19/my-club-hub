...
The app does not rely on a seeded admin account for production use. Create the first admin account using the setup route when no admins exist:

```
http://localhost:5000/setup
```

Alternatively, insert an admin directly into the database after creating a password hash with Werkzeug.
